//ANALISA SORTING DAN SEARCHING (SELECTION DAN INSERTION SORT + BINARY DAN INTERPOLATION SEARCH)
...
int main() {
    int menu,hapus,jumlah=0,proses=0,i,j,posisi,cari,tmp,flag,kiri,kanan,tengah,low,high;
    ...
    while (proses<1) {
        ...
        printf("5. Urutkan Data(insertion sort)\n");
		printf("6. Cari Data(sequence search)\n");
        printf("7. Cari Data(binary search)\n");
        printf("8. Cari Data(interpolation search)\n");
		printf("9. Hapus Elemen Array\n");
        printf("10. Mengembalikan Nilai Array\n");
        printf("11. Reset Data\n");
        printf("12. Keluar dari Program\n\n");
        ...
        if (menu==1) {
			...
			if (jumlah>0){
				...
				printf("\n\nHasil pengurutan insertion sort sebagai berikut:\n");
				for (i=0;i<=jumlah-1; i++){
				printf("%d ", nilai[i]);
				}
				printf("\n");				
    		...
    		if (jumlah>0){
	            ...
	            printf("\n\nmasukkan nilai data yang anda cari: ");
	            scanf("%d",&cari);
	            //masuk rumus interpolation
				//proses pencarian data
				tmp=0;
				high=9;
				low=0;
				while(cari>=nilai[low]&&cari<=nilai[high]){
					posisi=((cari-nilai[low])/(nilai[high]-nilai[low]))*(high-low)+low;
				...
				if (tmp>0){
					printf("data yang anda cari ADA");
				}
				else{
